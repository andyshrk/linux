...
static int
n2d_debugfs_open(struct inode *inode, struct file *file)
{
...
static ssize_t
n2d_debugfs_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
...
n2d_error_t
n2d_debugfs_dir_init(n2d_debugfs_dir_t *dir, struct dentry *root, const char *name)
{
...
n2d_error_t
n2d_debugfs_dir_crestefiles(n2d_debugfs_dir_t *dir, n2d_debug_info_t *list, int count, n2d_pointer data)
{
    int i;
    ...
    for (i = 0; i < count; i++) {
        ...
        node->entry = debugfs_create_file(list[i].name, mode,
                                          dir->root, node, &n2d_debugfs_operations);
...
n2d_error_t
n2d_debugfs_dir_removefiles(n2d_debugfs_dir_t *dir, n2d_debug_info_t *list, int count)
{
    int i;
    n2d_debug_node_t *node;
