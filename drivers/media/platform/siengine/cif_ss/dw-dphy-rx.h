...
#define CFG_CLK_NAME "cisp-dphy-cfg-clk"
...
#define DPHY_CFG_CLK_FREQ_MAX 27000000
#define DPHY_CFG_CLK_FREQ_MIN 17000000
#define DPHY_CFG_CLK_FREQ_DEFAULT 25000000
...
enum dphy_12bit_interface_addr {
	...
	RX_RX_DUAL_PHY      = 0x133,   //master or slave
	RX_CLKLANE_LANE_3   = 0x304,   //clock lane low power detector
	RX_CLKLANE_LANE_4   = 0x305,   //ULP mode and low power receiver
	RX_CLKLANE_LANE_6   = 0x307,   //rx hs clock to long channel, clock lane contrl
	RX_CLKLANE_LANE_7   = 0x308,   //clock lane source selection
	RX_LANE0_LANE_7     = 0x508,   //lane0 ctrl, HS clock source from long channel
	RX_LANE1_LANE_7     = 0x708,   //lane1 ctrl
	RX_LANE0_DDL_0      = 0x606,
};
...
struct dw_dphy_rx {
	...
	u8 lane_usebit; /* which lanes in use at this phy */
	bool master;	/* in slave module, some operation still at master phy */
	int idx;		/* which phy */
	struct dw_dphy_rx *partner; /* if 4lane/3lane, partner is other phy */

	struct siengine_cif_misc *cif_misc;
	spinlock_t clk_lock;
	struct clk *clk_cfg;
};
...
{
	/* each d-phy support not more than 2 lanes */
	unsigned int lane_usebit = args->args[0] & 0x3;
	struct dw_dphy_rx *dphy = dev_get_drvdata(dev);

	dphy->lane_usebit = lane_usebit;
	return dphy->phy;
}
