...
struct range_dphy_gen3 range_gen3[] = {
	{ 80, 0x00, 0x1C0 }, { 90, 0x10, 0x1B6 }, { 100, 0x20, 0x1B6 },
	{ 110, 0x30, 0x1C0 }, { 120, 0x01, 0x1B6 }, { 130, 0x11, 0x1B6 },
	{ 140, 0x21, 0x1C0 }, { 150, 0x31, 0x1B6 }, { 160, 0x02, 0x1B6 },
	{ 170, 0x12, 0x1C0 }, { 180, 0x22, 0x1B6 }, { 190, 0x32, 0x1C0 },
	{ 205, 0x03, 0x1C0 }, { 220, 0x13, 0x1B6 }, { 235, 0x23, 0x1B6 },
	{ 250, 0x33, 0x1C0 }, { 275, 0x04, 0x1B6 }, { 300, 0x14, 0x1C0 },
	{ 325, 0x25, 0x1C0 }, { 350, 0x35, 0x1B6 }, { 400, 0x05, 0x1B6 },
	{ 450, 0x16, 0x1C0 }, { 500, 0x26, 0x1C0 }, { 550, 0x37, 0x1B6 },
	{ 600, 0x07, 0x1C0 }, { 650, 0x18, 0x1B6 }, { 700, 0x28, 0x1B6 },
	...
	{ 1500, 0x2C, 0x1C0 }, { 1550, 0x3D, 0x10F }, { 1600, 0x0D, 0x118 },
...
static inline bool cif_misc_dual_dphy(struct dw_dphy_rx *dphy)
{
	return (dphy->master && dphy->partner);
}

static u32 cif_misc_read(struct dw_dphy_rx *dphy, u32 address)
{
	return ioread32(dphy->cif_misc->base_address + address);
}

static void cif_misc_write(struct dw_dphy_rx *dphy, u32 address, u32 data)
{
	iowrite32(data, dphy->cif_misc->base_address + address);
}

static inline void cif_misc_work_mode(struct dw_dphy_rx *dphy)
{
	u32 workmode;
	unsigned long flags;

	spin_lock_irqsave(&dphy->cif_misc->lock, flags);

	workmode = cif_misc_read(dphy, CSI_WORK_MODE);
	if (cif_misc_dual_dphy(dphy))
		workmode |= 1 << (dphy->idx >> 1);
	else
		workmode &= ~(1 << (dphy->idx >> 1));

	cif_misc_write(dphy, CSI_WORK_MODE, workmode);

	spin_unlock_irqrestore(&dphy->cif_misc->lock, flags);
}

static void cif_misc_forcerxmode(struct dw_dphy_rx *dphy, bool enable)
{
	u32 stop;
	u32 work_lanes = dphy->lane_usebit;
	u32 work_mask = 0x3;
	u32 shift = (dphy->idx << 1);
	unsigned long flags;
	...
	stop = cif_misc_read(dphy, CSI_DPHY_FORCERXMODE);
	...
	cif_misc_write(dphy, CSI_DPHY_FORCERXMODE, stop);

	spin_unlock_irqrestore(&dphy->cif_misc->lock, flags);
}

static void cif_misc_clock_en(struct dw_dphy_rx *dphy, bool enable)
{
	u32 clock;
	unsigned long flags;
	...
	clock = cif_misc_read(dphy, CSI_DPHY_CLOCK_EN);
	...
	cif_misc_write(dphy, CSI_DPHY_CLOCK_EN, clock);
...
u8 dw_dphy_setup_config(struct dw_dphy_rx *dphy)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (ret < 0) {
			dev_vdbg(&dphy->phy->dev, "could not acquire config (err=%d)\n", ret);
			return ret;
		}
	...
	/*
	 * if 1lane or 2lane, use CTRL_2_LANES, single d-phy mode
	 * if 3lane or 4lane, use CTRL_4_LANES, dual d-phy mode
	 */
	return dphy->partner ? CTRL_4_LANES : CTRL_2_LANES;
#endif
...
{
	u32 ctrl0, ctrl1;
	struct dw_dphy_rx *master = dphy;

	/* if slave phy IO operation, get master */
	if (dphy->master) {
		ctrl0 = R_CSI2_DPHY_TST_CTRL0;
		ctrl1 = R_CSI2_DPHY_TST_CTRL1;
	} else {
		master = dphy->partner;

		ctrl0 = R_CSI2_DPHY2_TST_CTRL0;
		ctrl1 = R_CSI2_DPHY2_TST_CTRL1;
	}

	dw_dphy_write_msk(master, ctrl1, 0, PHY_TESTEN, 1);
	dw_dphy_write_msk(master, ctrl0, 0, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl1, 1, PHY_TESTEN, 1);
	dw_dphy_write_msk(master, ctrl0, 1, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl1, 0x00, PHY_TESTDIN, 8);
	dw_dphy_write_msk(master, ctrl0, 0, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl1, 0, PHY_TESTEN, 1);
	dw_dphy_write_msk(master, ctrl1, (u8)(addr >> 8), PHY_TESTDIN, 8);
	dw_dphy_write_msk(master, ctrl0, 1, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl0, 0, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl1, 1, PHY_TESTEN, 1);
	dw_dphy_write_msk(master, ctrl0, 1, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl1, (u8)addr, PHY_TESTDIN, 8);
	dw_dphy_write_msk(master, ctrl0, 0, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl1, 0, PHY_TESTEN, 1);
	dw_dphy_write_msk(master, ctrl1, (u8)data, PHY_TESTDIN, 8);
	dw_dphy_write_msk(master, ctrl0, 1, PHY_TESTCLK, 1);
	dw_dphy_write_msk(master, ctrl0, 0, PHY_TESTCLK, 1);
}
...
static int dw_dphy_te_12b_read(struct dw_dphy_rx *dphy, u32 addr)
{
	...
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL0, 0, PHY_TESTCLK, 1);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL1, 0, PHY_TESTEN, 1);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL1, (u8)(addr >> 8), PHY_TESTDIN, 8);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL0, 1, PHY_TESTCLK, 1);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL0, 0, PHY_TESTCLK, 1);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL1, 1, PHY_TESTEN, 1);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL0, 1, PHY_TESTCLK, 1);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL1, (u8)addr, PHY_TESTDIN, 8);
	dw_dphy_write_msk(dphy, R_CSI2_DPHY_TST_CTRL0, 0, PHY_TESTCLK, 1);
...
/* if dual d-phy mode, master d-phy wakeup fisrt, and after wake up slave d-phy */
static int dw_dphy_wakeup(struct dw_dphy_rx *dphy)
{
	u32 status;
	u32 testbits;

	testbits = ((u32)dphy->lane_usebit) << (dphy->idx << 1);
	if (dphy->master)
		testbits |= 1 << 16;

	return read_poll_timeout_atomic(dw_dphy_read, status,
		status & testbits, 100, 50000, false, dphy, R_CSI2_DPHY_STOPSTATE);
}
...
static int dw_dphy_gen3_12bit_configure(struct dw_dphy_rx *dphy)
{
	...
	if (dphy->phy_type || (input_freq / 1000) <= 1500)
		dw_dphy_te_write(dphy, RX_SYS_7, 0x38);
	if (cif_misc_dual_dphy(dphy)) {
		dw_dphy_te_write(dphy->partner, RX_SYS_1, range_gen3[range].hsfregrange);
		dw_dphy_te_write(dphy->partner, RX_SYS_0, 0x20);
		dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_2,
				 (u8)range_gen3[range].osc_freq_target);
		dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_3,
				 (u8)(range_gen3[range].osc_freq_target >> 8));
		dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_4, 0x01);

		if (dphy->partner->phy_type) {
			dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_1, 0x01);
			dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_0, 0x80);
		}

		dw_dphy_te_write(dphy->partner, RX_SYS_7, 0x38);
	}

	dw_dphy_te_write(dphy, RX_LANE0_DDL_0, 0x20);      //enable rx lane0 ddl
	if (cif_misc_dual_dphy(dphy))
		dw_dphy_te_write(dphy->partner, RX_LANE0_DDL_0, 0x20);
...
static int dw_dphy_configure(struct dw_dphy_rx *dphy)
{
	...
	/* stop all lanes */
	cif_misc_forcerxmode(dphy, false);
	if (cif_misc_dual_dphy(dphy))
		cif_misc_forcerxmode(dphy->partner, false);

	cif_misc_clock_en(dphy, true);
	...
	if (dphy->dphy_gen == GEN3) {
		...
		if (dphy->dphy_te_len == BIT12) {
			...
			if (cif_misc_dual_dphy(dphy)) {
				dw_dphy_write_msk(dphy, R_CSI2_DPHY2_TST_CTRL0, 1, PHY_TESTCLR, 1);
				dw_dphy_write_msk(dphy, R_CSI2_DPHY2_TST_CTRL0, 0, PHY_TESTCLR, 1);
			}
	...
	if (cif_misc_dual_dphy(dphy)) {
		dw_dphy_te_write(dphy, RX_RX_DUAL_PHY, 1);
		dw_dphy_te_write(dphy->partner, RX_RX_DUAL_PHY, 0);
		dw_dphy_te_write(dphy, RX_CLKLANE_LANE_6, 4);
		dw_dphy_te_write(dphy->partner, RX_CLKLANE_LANE_6, 0);
		dw_dphy_te_write(dphy, RX_LANE0_LANE_7, 0x20);
		dw_dphy_te_write(dphy->partner, RX_LANE0_LANE_7, 0x20);
		dw_dphy_te_write(dphy, RX_LANE1_LANE_7, 0x20);
		dw_dphy_te_write(dphy->partner, RX_LANE1_LANE_7, 0x20);
		dw_dphy_te_write(dphy, RX_CLKLANE_LANE_7, 0);
		dw_dphy_te_write(dphy->partner, RX_CLKLANE_LANE_7, 0x08);
		dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_0, 0x03);
		dw_dphy_te_write(dphy->partner, RX_RX_STARTUP_OVR_1, 0x02);
		dw_dphy_te_write(dphy->partner, RX_CLKLANE_LANE_6, 0x08);
		dw_dphy_te_write(dphy->partner, RX_CLKLANE_LANE_3, 0x80);
		dw_dphy_te_write(dphy->partner, RX_CLKLANE_LANE_4, 0x0a);
	}

	udelay(1000);
	dw_dphy_pwr_up(dphy);
	if (cif_misc_dual_dphy(dphy))
		dw_dphy_write(dphy, R_CSI2_DPHY_SHUTDOWNZ, 3);

	/* master dphy */
	ret = dw_dphy_wakeup(dphy);
	if (ret) {
		dev_err(&dphy->phy->dev, "master d-phy wake up timeout\n");
		return ret;
	}
	dev_info(&dphy->phy->dev, "master d-phy wake up success\n");

	/* turn on master d-phy lanes */
	cif_misc_forcerxmode(dphy, true);

	if (cif_misc_dual_dphy(dphy)) {
		dw_dphy_write(dphy, R_CSI2_DPHY_RSTZ, 3);

		ret = dw_dphy_wakeup(dphy);
		if (ret) {
			dev_err(&dphy->partner->phy->dev, "slave d-phy wake up timeout\n");
			return ret;
		}

		dev_info(&dphy->partner->phy->dev, "slave d-phy wake up success\n");

		/* turn on slave d-phy lanes */
		cif_misc_forcerxmode(dphy->partner, true);
	}
...
int dw_dphy_if_set_idelay(struct dw_dphy_rx *dphy, u8 dly, u8 cells)
{
	...
	if (val != dly) {
		dev_vdbg(&dphy->phy->dev, "odelay config failed, set %d get %d", dly, val);
		return -EINVAL;
	}
...
int dw_dphy_init(struct phy *phy)
{
	...
	dev_warn(&dphy->phy->dev, "DPHY Init.\n");
	//put to dphy power on to resolve after resume PKT FATAL
	//cif_misc_work_mode(dphy);
	return 0;
}
...
static int dw_dphy_set_phy_state(struct dw_dphy_rx *dphy, u32 on)
{
	...
	/* dual lane mode, slave dphy init by master dphy */
	if (!dphy->master) {
		dev_info(&dphy->phy->dev,
			"Init by master dphy : %pOF\n", dphy->partner->phy->dev.of_node);
...
int dw_dphy_power_off(struct phy *phy)
{
	...
	dev_info(&dphy->phy->dev, "DPHY Power OFF\n");
...
int dw_dphy_reset(struct phy *phy)
{
	...
	if (cif_misc_dual_dphy(dphy))
		dw_dphy_reset(dphy->partner->phy);
