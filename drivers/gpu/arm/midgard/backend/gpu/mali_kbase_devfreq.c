...
static int max_core (void *kbdev) {
	static int core_num;
	struct kbase_device * pkbdev = (struct kbase_device *)kbdev;
	u64 full_mask = pkbdev->gpu_props.props.raw_props.shader_present;

	if (core_num) return core_num;

	for (; full_mask; core_num++, full_mask &= (full_mask - 1)) ;

	return core_num;
}
static void core_mask_set(void *args, uint64_t num) {
	struct kbase_device *kbdev = (struct kbase_device *) args;
	unsigned long flags;
	static u64 db[] = {
	0,
	GPU0_STACK0 ,
	GPU0_STACK0 | GPU0_STACK4 ,
	GPU0_STACK4 | GPU0_STACK6 ,
	GPU0_STACK0 | GPU0_STACK6 | GPU0_STACK4,
	GPU0_STACK1 | GPU0_STACK4 | GPU0_STACK6,
	GPU0_STACK0 | GPU0_STACK1 | GPU0_STACK4 | GPU0_STACK6,
	GPU0_STACK0 | GPU0_STACK1 | GPU0_STACK2 | GPU0_STACK6,
	GPU0_STACK0 | GPU0_STACK1 | GPU0_STACK2 | GPU0_STACK4 | GPU0_STACK6,
	GPU0_STACK0 | GPU0_STACK1 | GPU0_STACK2 | GPU0_STACK5 | GPU0_STACK6,
	GPU0_STACK0 | GPU0_STACK1 | GPU0_STACK2 | GPU0_STACK4 | GPU0_STACK5 | GPU0_STACK6
	};
	static u8 old_core_num = 10;
	int full_core = max_core(args);
	int new_core_num;
	uint64_t core_mask;
	int i;
	u64	 need_wa = 0;

	new_core_num = full_core - (int)num;

	if (new_core_num <= 0) {
		new_core_num = 1;
	}
	else if (new_core_num > 10) {
		new_core_num = 10;
	}

	if (old_core_num == new_core_num) return;

	core_mask = db[new_core_num];
	need_wa = kbdev->current_core_mask^core_mask;
	need_wa &= ~kbdev->current_core_mask;

	if (need_wa) {
		mutex_lock(&kbdev->ipa.lock);
		for (i = 0; i < kbdev->nr_clocks; i++) {
			if (kbdev->clocks[i]) {
				clk_set_rate(kbdev->clocks[i], POWERON_CORE_FREQ);
			}
		}
	}
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	...
	if (need_wa) {
		...
		for (i = 0; i < kbdev->nr_clocks; i++) {
			if (kbdev->clocks[i]) {
				clk_set_rate(kbdev->clocks[i], kbdev->current_nominal_freq);
			}
		}
		mutex_unlock(&kbdev->ipa.lock);
	}
	...
	old_core_num = new_core_num;

	return ;
}

static unsigned long dummy_max (void *args) {
	return 10;
}

static void set_work_core(void *args, uint64_t num) {
	struct kbase_device *kbdev = (struct kbase_device *) args;
	u64 shader_present, core_mask = 0;
	u64 group0_core_mask = 0;
	int core_num = 0;
	unsigned long flags;
	...
	for (; shader_present; core_num++, shader_present &= (shader_present- 1)) ;

	shader_present = kbdev->gpu_props.props.raw_props.shader_present;

	core_num = core_num - num ;

	if (core_num <= 0) return ;

	for (; core_num > 0; core_num--) {
		int core = ffs(shader_present);

		if (!core) {
			dev_err(kbdev->dev, "GPU does not have enough cores\n");
			return ;
		}
		core_mask |= (1ull << (core-1));
		shader_present &= ~(1ull << (core-1));
	}

	if (kbdev->current_core_mask == core_mask) return ;
	...
	if ((core_mask & shader_present) != core_mask) {
		dev_err(kbdev->dev, "Invalid core mask 0x%llX for all JS: Includes non-existent cores (present = 0x%llX)",
		       core_mask, shader_present);
		goto unlock;
	} else if (!(core_mask & shader_present & kbdev->pm.backend.ca_cores_enabled)) {
		dev_err(kbdev->dev, "Invalid core mask 0x%llX for all JS: No intersection with currently available cores (present = 0x%llX, CA enabled = 0x%llX\n",
		       core_mask,
		       kbdev->gpu_props.props.raw_props.shader_present,
		       kbdev->pm.backend.ca_cores_enabled);
		goto unlock;
	} else if (!(core_mask & group0_core_mask)) {
		dev_err(kbdev->dev, "Invalid core mask 0x%llX for all JS: No intersection with group 0 core mask 0x%llX\n",
		        core_mask, group0_core_mask);
		goto unlock;
	}

	if (kbdev->pm.debug_core_mask[0] != core_mask ||
	    kbdev->pm.debug_core_mask[1] != core_mask ||
	    kbdev->pm.debug_core_mask[2] != core_mask ||
	    kbdev->current_core_mask != core_mask) {
...
int init_thermal_thread(void * data)
{
	struct kbase_device *kbdev = (struct kbase_device *)data;

	struct gpu_ops d = {
		.max_state = dummy_max,
		.set_cooling = kbdev->reg_start == GPU0_REG_BASE ? core_mask_set : set_work_core,
		.args = (void *)kbdev
	};
	gpu_cooling_register(kbdev->dev->of_node, &d,  "thermal-gpu-core");
