...
void kbase_pm_do_poweron(struct kbase_device *kbdev, bool is_resume)
{
	...
	int i;
	if ((kbdev->reg_start == GPU0_REG_BASE) &&
	    clk_rate == 0 && kbdev->nr_clocks > 0)
		clk_rate = clk_get_rate(kbdev->clocks[0]);
#endif
...
#ifndef CONFIG_MALI_DEVFREQ
	if ((kbdev->reg_start == GPU0_REG_BASE) &&
	    (kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_SHADER) != 0x0))
		for (i = 0; i < kbdev->nr_clocks; i++) {
			if (kbdev->clocks[i]) {
				clk_set_rate(kbdev->clocks[i], clk_rate);
			}
		}
#endif
}
