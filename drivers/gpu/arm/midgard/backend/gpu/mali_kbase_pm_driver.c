					...
					enum kbasep_pm_action action)
{
	...
	if (kbase_dummy_job_wa_enabled(kbdev) &&
		...
		if (action == ACTION_PWRON &&
		   kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_SHADER) == 0 &&
		   core_type == KBASE_PM_CORE_SHADER &&
		   kbdev->gpu_props.props.raw_props.shader_present == 0x670067)
			...
			for (i = 0; i < kbdev->nr_clocks; i++) {
				if (kbdev->clocks[i]) {
					...
					kbdev->current_nominal_freq = POWERON_CORE_FREQ;
					kbdev->current_freqs[i] = POWERON_CORE_FREQ;
					kbdev->current_core_mask = cores;
#endif
				}
			}
