...
static ssize_t set_core_mask(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!kbdev)
		return -ENODEV;
	old_mask = kbdev->pm.debug_core_mask_all;
	...
	new_mask = new_core_mask[1] | new_core_mask[2] | new_core_mask[0];
	need_wa = old_mask^new_mask;
	need_wa &= ~old_mask;
	mutex_lock(&kbdev->pm.lock);
	if (need_wa &&
	    kbdev->gpu_props.props.raw_props.shader_present == 0x670067) {
		int i;
		mutex_lock(&kbdev->ipa.lock);
		if (kbdev->clocks[0])
			freq = clk_get_rate(kbdev->clocks[0]);
		for (i = 0; i < kbdev->nr_clocks; i++)
			if (kbdev->clocks[i]) clk_set_rate(kbdev->clocks[i], POWERON_CORE_FREQ);
	}
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	...
	if (kbdev->pm.debug_core_mask[0] != new_core_mask[0] ||
		...
		kbdev->current_core_mask = kbdev->pm.debug_core_mask_all;
#endif
	}
#endif /* MALI_USE_CSF */
...
unlock:
	...
	if (need_wa &&
		...
		mdelay(100);
		for (i = 0; i < kbdev->nr_clocks; i++)
			if (kbdev->clocks[i]) clk_set_rate(kbdev->clocks[i], freq);
				...
				struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "utilisation:	%d\n"
			    "current frequence: %lu\n"
			    "current core mask: 0x%llx\n",
			    utilisation, kbdev->current_nominal_freq,
			    kbdev->current_core_mask);
#else
	return sprintf(buf, "Do not support this function "
			    "unless config DEVFREQ or DVFS\n");
#endif
...
#endif
	for (i = 0; i < kbdev->nr_clocks; i++)
		if (kbdev->clocks[i] && __clk_is_enabled(kbdev->clocks[i]))
			clk_disable_unprepare(kbdev->clocks[i]);
	return 0;
}
...
static int kbase_device_resume(struct device *dev)
{
	...
	/* delay 10ms for waiting GPU active cores stable */
	msleep(10);
	...
	if (kbdev->devfreq) {
		mutex_lock(&kbdev->pm.lock);
		/* In these cases, devfreq function could not resume.
		 * FIRST: GPU does no task. When OS back from STR,
		 * GPU active_count is zero, because no task need be handled.
		 * SECOND: OS stay in STR too long time, GPU would not handle
		 * the task because task timeout. active_count is also zero.
		 * So change the condition to shader core ready.
		 * */
		//if (kbdev->pm.active_count > 0)
		if (kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_SHADER) != 0)
