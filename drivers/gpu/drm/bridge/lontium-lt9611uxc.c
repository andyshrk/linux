...
#define LT9611_DT_TIMING
#ifdef LT9611_DT_TIMING
static const u32 lt9611_bus_formats[] = {
 ...
 */
static const struct display_timing lt9611_default_timing = {
...
static u8 LT9611UXC_V4p2s3_DSI_PortA_FW[] = {
#include "LT9611UXC_V4p2s3_DSI_PortA_2021062701.txt"
};
...
static int lt9611uxc_connector_get_modes(struct drm_connector *connector)
{
	...
	drm_display_mode_from_videomode(&lt9611uxc->vm, mode);
	mode->width_mm = lt9611uxc->width_mm;
	mode->height_mm = lt9611uxc->height_mm;
	connector->display_info.width_mm = lt9611uxc->width_mm;
	connector->display_info.height_mm = lt9611uxc->height_mm;
	mode->type = DRM_MODE_TYPE_DRIVER | DRM_MODE_TYPE_PREFERRED;


	if (lt9611uxc->vm.flags & DISPLAY_FLAGS_DE_HIGH)
		*bus_flags |= DRM_BUS_FLAG_DE_HIGH;
	if (lt9611uxc->vm.flags & DISPLAY_FLAGS_DE_LOW)
		*bus_flags |= DRM_BUS_FLAG_DE_LOW;
	if (lt9611uxc->vm.flags & DISPLAY_FLAGS_PIXDATA_NEGEDGE)
		*bus_flags |= DRM_BUS_FLAG_PIXDATA_NEGEDGE;
	if (lt9611uxc->vm.flags & DISPLAY_FLAGS_PIXDATA_POSEDGE)
		*bus_flags |= DRM_BUS_FLAG_PIXDATA_POSEDGE;

	ret = drm_display_info_set_bus_formats(&connector->display_info,
			lt9611_bus_formats, ARRAY_SIZE(lt9611_bus_formats));
	if (ret) {
...
err_remove_bridge:
	free_irq(lt9611uxc->client->irq, lt9611uxc);
			      ...
			      struct lt9611uxc *lt9611uxc)
{
	...
	if (timings) {
		of_node_put(timings);
		ret = of_get_videomode(dev->of_node, &lt9611uxc->vm, 0);
	} else {
		videomode_from_timing(&lt9611_default_timing, &lt9611uxc->vm);
	}
	...
	of_property_read_u32(dev->of_node, "panel-width-mm", &lt9611uxc->width_mm);
	of_property_read_u32(dev->of_node, "panel-height-mm", &lt9611uxc->height_mm);
#endif
...
static int lt9611uxc_firmware_update(struct lt9611uxc *lt9611uxc)
{
	...
	fw->data = LT9611UXC_V4p2s3_DSI_PortA_FW;
	fw->size = sizeof(LT9611UXC_V4p2s3_DSI_PortA_FW);
...
retry:
	...
	lt9611uxc->bridge.type = DRM_MODE_CONNECTOR_DSI;
