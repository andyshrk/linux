...
static int se1000_aipu_clk_dt(struct platform_device *pdev, struct aipu_priv *aipu)
{
	aipu->npu_clk = devm_clk_get(&pdev->dev, NULL);
	if (IS_ERR(aipu->npu_clk)) {
		dev_warn(&pdev->dev, "could not get clk\n");
		return PTR_ERR(aipu->npu_clk);
	}

	aipu->max_freq = clk_get_rate(aipu->npu_clk);
	if (!aipu->max_freq)
		return -EINVAL;
			...
			struct aipu_soc_operations *ops)
{
	...
	if (!aipu) {
		...
		if (ret)
			return ret;
		ret =  se1000_aipu_clk_dt(p_dev, aipu);
		if (ret)
			return ret;
	}
...
int armchina_aipu_suspend(struct platform_device *p_dev, pm_message_t state)
{
	...
	if (aipu && aipu->npu_clk) {
		aipu->cur_freq = clk_get_rate(aipu->npu_clk);
		if (!aipu->cur_freq)
			return -EINVAL;
	}
	return 0;
...
int armchina_aipu_resume(struct platform_device *p_dev)
{
	...
	if (aipu && aipu->npu_clk) {
		...
		if (!aipu->max_freq)
			return -EINVAL;
		if (aipu->max_freq != aipu->cur_freq) {
			ret = clk_set_rate(aipu->npu_clk, aipu->max_freq);
			if (ret)
				return ret;
		}
		ret = clk_set_rate(aipu->npu_clk, aipu->cur_freq);
		if (ret)
			return ret;
	}
